...
// "The first step in convolution operation is multiplying each input to
// the corresponding pixel in the filter by using DSPs ... After the
// multipliers, the pipelining registers are employed to reduce the
// timing path."
//
// Sec 5.3.2.1 / 5.4.1.1 / 5.4.1.2 / 5.5.1.1 (all identical):
// "The multipliers double the number of bits from 15 to 30 then the 7
// LSBs are thrown away as quantization noise."
//
// Bit-format details:
...
module mac_unit #(
    ...
    // ------------- Drop the 7 LSBs ( Sec 5.3.2.1 / 5.4.1.1) ----
    ...
    // the bottom DROP_LSB bits,
    // "the 7 LSBs are thrown away as quantization noise".
    wire signed [MUL_OUT_W-1:0] product_truncated =
        product_full[MUL_FULL_W-1 : DROP_LSB];

    // ------------- Pipeline register ( Sec 5.3.2.1) ------------
    // Clock-gated by `en` for dynamic-power savings (Sec 5.4.1.1).
