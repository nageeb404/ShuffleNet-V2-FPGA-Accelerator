...
// ROM test: g2_dw_bias_rom (distributed/combinational, 16 entries, 870 bits)
module tb_g2_dw_bias_rom;
    reg [3:0] addr=0;
    wire [869:0] data_out;
    g2_dw_bias_rom dut(.addr(addr),.data_out(data_out));
    integer pass_cnt=0, fail_cnt=0, total=0;
    initial begin
        $display("=== tb_g2_dw_bias_rom ===");
        #2;
        total=total+1;
        ...
        addr=15; #2;
        total=total+1;
        if (^data_out===1'bx) begin $display("FAIL addr=15 X"); fail_cnt=fail_cnt+1; end
        else pass_cnt=pass_cnt+1;
