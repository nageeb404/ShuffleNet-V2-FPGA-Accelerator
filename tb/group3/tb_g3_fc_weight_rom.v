...
// ROM test: g3_fc_weight_rom (BRAM, 32000 entries, 288 bits each)
module tb_g3_fc_weight_rom;
    reg clk=0; always #5 clk=~clk;
    reg [14:0] addr=0;
    wire [287:0] data_out;
    g3_fc_weight_rom dut(.clk(clk),.addr(addr),.data_out(data_out));
    integer pass_cnt=0, fail_cnt=0, total=0;
    initial begin
        $display("=== tb_g3_fc_weight_rom ===");
